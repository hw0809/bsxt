...
<body>
							...
							<div class="widget-body">
								<div class="widget-main">
									...
									<form class="form-horizontal" role="form" id="commitForm">
										<input type="hidden" value="${classes.id}" id="id" name="id">
										...
										<div class="form-group">
											...
											<div class="col-sm-4">
												<input type="text" id="name" name="name" placeholder="班级" value="${classes.name}"
													class="form-control" />
											</div>
										</div>
	...
	<script type="text/javascript">
		var code = "${classes.department.departmentCode}";
		var grade = "${classes.grade}";
		...
		var initGrade = function(studentGrade) {
			var date = new Date();
			...
			$("#grade").val(studentGrade);
		};

		var submitForm = function() {
			if (!$("#name").val()) {
				window.parent.myToastrError("请输入班级");
				$("#name").focus();
				return;
			}
				...
				$.ajax({
					...
					success : function(data) {
						if (data.flag) {
							...
							refreshTab("tab_manageClassesId");
							closeTab("tab_eidtClassesId");
						} else {
