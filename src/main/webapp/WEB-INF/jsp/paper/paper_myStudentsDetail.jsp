								...
								<div class="widget-main">
									<form class="form-horizontal" role="form" id="commitForm">
										...
										<div class="form-group">
											...
											<div class="col-sm-2">
												<input type="text" id="account" name="account" value="${student.account}"
													placeholder="学号" readonly="" class="form-control" />
											</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-4">
												<textarea readonly="" class="form-control"
													placeholder="简介" id="desc" name="desc" rows="5"
													maxlength="50">${student.desc}</textarea>
											</div>
										</div>
						...
						<div class="clearfix form-actions">
							<div class="col-md-offset-3 col-md-9">
								...
								<button class="btn" type="button"
									onclick="returnForm()">
									<i class="ace-icon fa fa-times bigger-110"></i> 退回
								</button>
							</div>
						</div> 
	...
	<script type="text/javascript">
			...
			$.ajax({
				...
				success : function(result) {
					if (result.flag) {
						...
						setTimeout("refreshTab('tab_myStudentsPaperId')",2000);
						setTimeout("closeTab('tab_myStudentsDetailId')",2000);
					}
				}
			});
		};
		...
		var returnForm = function() {
			$.ajax({
				...
				success : function(result) {
					if (result.flag) {
						window.parent.myToastrSuccess("已退回学生选题,2秒后页面关闭");
						setTimeout("refreshTab('tab_myStudentsPaperId')",2000);
						setTimeout("closeTab('tab_myStudentsDetailId')",2000);
					}
				}
