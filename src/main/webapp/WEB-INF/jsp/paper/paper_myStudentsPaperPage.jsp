	...
	<script type="text/javascript">
		...
		jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				//url : "${dormitory}/paper/getPageList",
				 url : "${dormitory}/paper/getMyStudentsPageList",
				height : 250,
				datatype : "json",
				colNames : [ 'id', '论文题目', '学生', '年份', '班级', '限制选人数', '审核' ],
				colModel : [ {
					name : 'id',
					index : 'id',
					editable : true,
					...
					formatter : function(cellvalue, options, rowObject) {
						...
						for (var i = 0; i < infos.length; i++) {
							var sinfo = infos[i];
							var sinfos = sinfo.split("##");
							linkStr += "<a onclick='studentDetail(" + sinfos[0] +")'>" + sinfos[1] + "</a>,";
						}
		...
		var del = function() {
			var delId = jQuery(grid_selector).jqGrid('getGridParam','selrow');
			
			if (delId == null) {
				window.parent.myToastrError("请选择一行");
			}
			
			$.ajax({
				url : "${dormitory}/paper/del",
				data : {
					id : delId,
				},
				type : "post",
				...
				success : function(result) {
					if (result.flag) {
						window.parent.myToastrSuccess(result.msg);
						location.reload();
					} else {
						window.parent.myToastrError(result.msg);
					}
				}
			}); 
		};
		...
		var studentDetail = function(id) {
			var url = "paper/getMyStudentsDetail?id=" + id;
			addTabs({
				id: 'myStudentsDetailId',
				title: '论文详情',
				close: true,
				url: url,
