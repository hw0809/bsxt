...
<body>
				...
				<div class="row">
					<div class="col-xs-12">
						<form id="submitForm" class="form-horizontal" role="form">
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" placeholder="标题" name="title" id="title"
										readonly="" value="${student.thesisTitle }"
							...
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right"
									for="form-field-1"> 分数 </label>

								<div class="col-sm-5">
									<input type="text" placeholder="分数" name="score" id="score"
										value="${student.thesisScore }" class="col-xs-10 col-sm-5" />
								</div>
							</div>

							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right"
									for="evaluate">评价</label>
								<div class="col-sm-5">
									<textarea class="form-control" id="evaluate" name="evaluate"
										placeholder="评价" rows="5" maxlength="50">${student.thesisEvaluate }</textarea>
								</div>
							</div>
						...
						<div class="clearfix form-actions">
							<div class="col-md-offset-3 col-md-9">
								...
								&nbsp; &nbsp; &nbsp;
								<button class="btn btn-info" type="button"
									onclick="downThesis()">下载毕业论文</button>
							</div>
						</div>
					</div>
				</div>
	...
	<script type="text/javascript">
		var downThesis = function() {
			window.location.href = "${dormitory}/paper/downThesisReport?account=${student.account}";
		};
		...
		var submitForm = function() {
			...
			if (score) {
				...
				if (score > 100 || score < 0) {
					window.parent.myToastrError("请输入正确的分数");
					return;
				}
			}

			$.ajax({
				url : "${dormitory}/paper/saveThesisEvaluate",
				type : "post",
				dataType : "json",
				data : $("#submitForm").serialize(),
				success : function(data) {
					window.parent.myToastrSuccess("保存成功");
				}
			});
		};
	</script>
