...
<body>
										...
										<div class="form-group">
											...
											<div class="col-sm-2">
												<input type="text" id="score" name="score" placeholder="分数"
													value="${student.reviewScore }" class="form-control" />
											</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-5">
												<textarea class="form-control" id="evaluate" name="evaluate"
													placeholder="评价" rows="5" maxlength="50">${student.reviewEvaluate }</textarea>
											</div>
										</div>
	...
	<script type="text/javascript">
		...
		var submitForm = function() {
			if (!$("#score").val()) {
				window.parent.myToastrError("请输入分数");
			}

			if ($("#score").val() < 0) {
				window.parent.myToastrError("请输入正确分数");
			}
			...
			$.ajax({
				url : "${dormitory}/reviewTeacherGroup/saveReviewStudentScore",
				type : "post",
				data : $("#commitForm").serialize(),
				dataType : "json",
				success : function(result) {
					if (result.flag) {
						window.parent.myToastrSuccess("已保存");
					}
				}
			});
		};
