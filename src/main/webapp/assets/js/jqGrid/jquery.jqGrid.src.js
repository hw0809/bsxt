...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		// add by yuyi 修复shrinkToFit=true的时候出现滚动条
		$(grid.bDiv)
		...
		if(ts.p.shrinkToFit){
			$(grid.bDiv).css({"overflow-x" : "hidden" });
		}
		...
		if( ts.p.toolbar[0] ) {
			...
			if(ts.p.toolbar[1] === "top") {
                ...
                //add by yuyi 添加顶部工具栏toolbar
				var e =$('<div class="btn-toolbar" style="padding:4px;margin-left:1px; background-image:linear-gradient(to bottom, #ffffff 0%, #eeeeee 100%);border-top:1px solid #CCC"></div>');
				e.append('<div class="btn-group"></div>');
				$(grid.uDiv).append(e);
			    $(grid.uDiv).attr("id", "t_" + this.id);
			}
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			if($t.p.toolbar[0] === true){
				...
				//add by yuyi
				var findnav = $(".btn-toolbar","#t_" + this.id)[0];
				$(findnav).css("width",nwidth+"px");
...
$.jgrid.extend({
	...
	navButtonAdd : function (elem, p) {
		...
		return this.each(function() {
			...
			//add by yuyi 按钮移到顶部工具栏
			//var findnav = $(".navtable", elem)[0], $t = this;
			var findnav = $(".btn-group","#t_" + this.id)[0], $t = this;
			if (findnav) {
				if( p.id && $("#"+$.jgrid.jqID(p.id), findnav)[0] !== undefined )  {return;}
				var tbd = $("<button></button>");
				if(p.id) {$(tbd).attr("id",p.id);}
				
				if(p.buttonicon.toString().toUpperCase() === "NONE") {
                    $(tbd).addClass('btn  btn-white ').html( p.caption );
				} else	{
				    $(tbd).addClass('btn  btn-white ').append("<i class='" + p.buttonicon + "'></i>" + p.caption +"");
				}
				if(p.id) {$(tbd).attr("id",p.id);}
				$(findnav).append(tbd);
				$(tbd,findnav)
	...
	navNasButtonAdd : function (elem, p) {	// add by pxy
		p = $.extend({
			caption : "newButton",
			title: "",
			id:"",
			...
			showrequestauth : "",	// 如果该按钮需要权限判断，设置该值
			onClickButton: null,
			position : "last",
			cursor : "pointer"
		}, p ||{});
		return this.each(function() {
			...
			if (findnav) {
				...
				if(p.id) {$(tbd).attr("id",p.id);}
				if(p.showrequestauth!=""){
					$(tbd).addClass(' validate-ui ');
					$(tbd).attr('targetvalidate',p.showrequestauth);
				}
